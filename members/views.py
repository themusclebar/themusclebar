...
def check_status(request, object):
    # print(object.registration_upto < datetime.datetime.today())
...
def export_all(user_obj):
    ...
    response["Content-Disposition"] = (
        'attachment; filename="' + first_name + " " + last_name + '.csv"'
    )
    return response
...
# Export user information.
def generateInvoice(pkid):
    try:
        order_db = Member.objects.get(pk=pkid)  # you can filter using order_id as well
    except:
        return HttpResponse("505 Not Found")
    data = {
        "member_id": order_db.unique_member_id,
        "user_email": order_db.email,
        "date": order_db.registration_date,
        "name": order_db.first_name + " " + order_db.last_name,
        "phone_no": order_db.mobile_number,
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            temp.registration_upto = parser.parse(
                ...
                days=(float(request.POST.get("subscription_period"))) * 30
            )
...
def update_member(request, id):
    ...
    if request.method == "POST" and request.POST.get("gym_membership"):
        ...
        if gym_form.is_valid():
            ...
            if (
                object.stop == 1
                and not request.POST.get("stop") == "0"
                and request.POST.get("gym_membership")
            ):
                messages.error(
                    request, "Please start the status of user to update the record"
                )
            ...
            #     str(object.start_date), "%Y-%m-%d"
            # ) != datetime.datetime.strptime(request.POST.get("start_date"), "%Y-%m-%d"):
            #     object.start_date = parser.parse(request.POST.get("start_date"))
            ...
            #     object.subscription_period = request.POST.get("subscription_period")
            #     object.start_date = parser.parse(request.POST.get("registration_upto"))
            #     object.registration_upto = parser.parse(
            ...
            #     object = check_status(request, object)
            #     model_save(object)
            # batch, amount, sub_per, start_date, reg_date
            elif (
                (object.batch != request.POST.get("batch"))
                or (object.amount != amount)
                or (
                    object.subscription_period
                    != request.POST.get("subscription_period")
                )
                or (
                    datetime.datetime.strptime(str(object.start_date), "%Y-%m-%d")
                ...
                or (
                    datetime.datetime.strptime(
                        str(object.registration_date), "%Y-%m-%d"
                    )
                    != datetime.datetime.strptime(
                        request.POST.get("registration_date"), "%Y-%m-%d"
                    )
                )
                or (request.POST.get("fee_status") == "paid")
                or (request.POST.get("fee_status") == "pending")
                or (
                    object.subscription_period
                    != request.POST.get("subscription_period")
                )
            ...
            # elif (object.amount != amount) and (
            #     (request.POST.get("fee_status") == "paid")
            #     or (request.POST.get("fee_status") == "pending")
            # ):
            #     object.amount = amount
            #     object.fee_status = request.POST.get("fee_status")
            ...
            # nothing is changed
                ...
                if not request.POST.get("stop") == "1":
                    object.start_date = parser.parse(
                        request.POST.get("registration_upto")
                    )
                    object.registration_upto = parser.parse(
                        request.POST.get("registration_upto")
            ...
            if object.fee_status == "paid":
                check = Payments.objects.filter(
                    payment_date=object.start_date, user__pk=object.pk
                ).count()
                if check == 0:
                    payments = Payments(
                        user=object,
                        payment_date=object.start_date,
