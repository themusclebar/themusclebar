...
class Member(models.Model):
    ...
    unique_member_id = models.CharField(("MEMBER's GYM ID"), max_length=50)
    ...
    start_date = models.DateField(("Start Date"), default="dd/mm/yyyy")
    ...
    fee_status = models.CharField(
        ("Fee Status"), max_length=30, choices=FEE_STATUS, default=FEE_STATUS[0][0]
    )
    ...
    class Meta:
        ...
        widgets = {
            "registration_date": forms.DateInput(
                attrs={"class": "datepicker form-control", "type": "date"}
            ),
...
class UpdateMemberGymForm(forms.Form):
    ...
    # subscription_upto = datetime.strptime(str(registration_upto), "%d/%m/%Y")
    # today = datetime.now()
    stop = forms.ChoiceField(label="Status", choices=STATUS)
    # if subscription_upto.date() < today.date():
    #     stop = "stop"
